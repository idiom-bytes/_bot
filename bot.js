...
bot.onText(/\/marketcap/, async (msg) => {
    try {
        ...
        let yfka_price = uniswap.ratioData["ETH_YFKA"] * coin_eth.getPrice("usd")
...
----------------------------------
XAMP Supply: ${numeral(coin_xamp.supplyCurrent["circulating"]).format(
            "0,0.00"
          )} | Price: $${numeral(coin_xamp.getPrice("usd")).format("0,0.0000")}
XAMP MCap $${numeral(
            coin_xamp.supplyCurrent["circulating"] * coin_xamp.getPrice("usd")
          ).format("0,0.00")}

TOB Supply: ${numeral(coin_tob.supplyCurrent["circulating"]).format(
            "0,0.00"
          )} | Price: $${numeral(coin_tob.getPrice("usd")).format("0,0.00")}
TOB MCap: $${numeral(
            coin_tob.supplyCurrent["circulating"] * coin_tob.getPrice("usd")
          ).format("0,0.00")}

BOA Supply: ${numeral(coin_boa.supplyCurrent["circulating"]).format(
            "0,0.00"
          )} | Price: $${numeral(coin_boa.getPrice("usd")).format("0,0.00")}
BOA MCap: $${numeral(
            coin_boa.supplyCurrent["circulating"] * coin_boa.getPrice("usd")
          ).format("0,0.00")}

YFKA Price ~= (Uniswap YFKA/ETH) * ETH/USD
YFKA Supply: ${numeral(coin_yfka.supplyCurrent["circulating"]).format(
            "0,0.00"
          )} | Price: $${yfka_price_formatted}
YFKA MCap: $${numeral(
            coin_yfka.supplyCurrent["circulating"] * yfka_price
          ).format("0,0.00")}
